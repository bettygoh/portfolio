...
(function () {
  ...
  const CSS = `
.pg-veil{position:fixed;inset:0;z-index:9999;display:grid;place-items:center;padding:24px;background:oklch(0.22 0.02 265 / 0.55);backdrop-filter:blur(6px);opacity:1;animation:pg-fade .3s var(--ease,ease) both}
.pg-veil.out{opacity:0;transition:opacity .28s var(--ease,ease)}
.pg-box{width:100%;max-width:400px;background:var(--bg,#fff);color:var(--ink,#111);border:1px solid var(--line,#e5e5e5);border-radius:20px;padding:clamp(24px,3vw,32px);box-shadow:0 40px 80px -40px oklch(0.2 0.04 265 / 0.5);animation:pg-rise .3s var(--ease,ease) both}
@keyframes pg-fade{from{opacity:0}to{opacity:1}}
@keyframes pg-rise{from{transform:translateY(8px) scale(0.985)}to{transform:none}}
...
.pg-input{font-size:16px;padding:14px 16px;min-height:50px}
.pg-row{gap:12px}
.pg-btn{min-height:48px;padding:13px 22px;font-size:15px}
}
`;
  ...
  function prompt(page, opts) {
    ...
    input.focus();
    const close = () => { veil.classList.add('out'); setTimeout(() => veil.remove(), 300); };
